...
void frame_tick() {
    ...
    if (pc != g_last_frame) { g_last_frame = pc; g_eye_counter = 0; advance_jitter(); }
}
...
void STDMETHODCALLTYPE hkOM(ID3D11DeviceContext *ctx, UINT n,
    ...
    // ---- Track scene target + frame/eye landmarks (both DLAA and upscale modes) ----
    if ((g_dlss_enabled || g_upscale_mode) && !InterlockedCompareExchange(&g_inflight, 0, 0)) {
        if (n > 0 && rtvs) {
...
void STDMETHODCALLTYPE hkVSCB(ID3D11DeviceContext *ctx, UINT start, UINT num,
    ...
    if (g_jitter_on && g_in_scene && g_shadow_valid && g_shadow_cb && bufs && num > 0 &&
        ...
        if (num <= 16) {
            ...
            if (sub) { g_orig_vscb(ctx, start, num, local); return; }
        }
    }
...
void STDMETHODCALLTYPE hkPSSRV(ID3D11DeviceContext *ctx, UINT start, UINT num,
    ...
    if ((g_dlss_enabled || g_upscale_mode) && g_scene_dirty && g_scene_color && g_om_dev &&
        ...
        if (hit && g_eye_counter < 2) {
            int eye = g_eye_counter;
            g_scene_dirty = false;
            g_eye_counter++;
            InterlockedExchange(&g_inflight, 1);
            __try {
